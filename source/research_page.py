...
<main class="project-main" id="main"><header class="project-header"><p class="eyebrow">MANUSCRIPT IN PREPARATION</p><h1>{h(manuscript['title'])}</h1><p class="project-authors">{authors}</p><p class="project-affiliations">Chung-Ang University · HUROTICS</p><div class="project-actions"><a class="cv-link" href="#video">Watch video</a><a class="cv-link" href="../../files/myomimetic-exosuit-video.mp4" download>Download video</a></div></header>
...
<section class="section" id="scope"><h2>Scope of the Study</h2><p>Evaluation covers steady-state indoor walking in seven healthy adults. Terrain transitions, patient-specific responses, and broader movement tasks are outside the reported validation. The simulation uses a planar musculoskeletal model, and simulated stair geometry differs from the experimental setup.</p><p class="media-caption">This research is in progress and has not yet been submitted.</p></section>
